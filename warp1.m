...
[X,Y] = meshgrid(y_inImg2,x_inImg2);
...
% final_X;
% 
% tranformed_Y = (Min_y+abs(Min_y)+1:Max_y)
% tranformed_X = (Min_x:Max_x)
% [X_tran,Y_tran]= meshgrid(tranformed_Y,tranformed_X);
% 
% size(X_tran)
% size(Y_tran)
% size(X)
% size(Y)
% g = interp2(X,Y,inImg2,X_tran,Y_tran,'linear',0);
